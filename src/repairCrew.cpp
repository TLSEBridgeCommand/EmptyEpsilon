    ...
    case RC_Idle:
        {
            ...
            if (system != SYS_None)
            {
                const float max_h = ship->systems[system].health_max;
                if (ship->systems[system].health < max_h)
                {
                    ship->systems[system].health += repair_per_second * delta;
                    if (ship->systems[system].health > max_h)
                        ship->systems[system].health = max_h;
                }
                if (ship->systems[system].health > max_h - 1e-4f)
                    ship->systems[system].health = max_h;
            ...
            if (ship->auto_repair_enabled && pos == target_position && (system == SYS_None || !ship->hasSystem(system) || ship->systems[system].health >= ship->systems[system].health_max - 1e-4f))
            {
                int n=irandom(0, SYS_COUNT - 1);

                if (ship->hasSystem(ESystem(n)) && ship->systems[n].health < ship->systems[n].health_max - 1e-4f)
                {
